...
```{r}
library(ROCR) # Plotting ROC Curve and Calculating AUC
...
Tujuan pemodelan merupakan membuat model yang tidak rumit (simple). Terlihat juga bahwa dengan ntree=200 dan mtry=13 yang secara relatif kecil, diperoleh AUC yang tinggi dibandingkan kombinasi parameter lainnya. Maka, akan digunakan parameter berikut.
```{r}
ntree_a = 150
depth_a = 13
ntree_b = 100
...
# Hasil Akhir
```{r}
data.frame("Model"=c("Regresi Logistic", "Naive Bayes", "GBM"), 
           "AUC Validasi Lokal" = round(c(logrAUC, nbAUC, gbmAUC),4),
           "AUC Pengumpulan" = c(0.8505, 0.7943, 0.8548))
```
